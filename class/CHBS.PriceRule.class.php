...
class CHBSPriceRule
{
	...
	function __construct()
	{
		...
		(
			1=>array(__('Set directly in rule','chauffeur-booking-system')),
	...
	{	  
		...
		foreach($option['pickup_time']['start'] as $index=>$value)
		{
			...
			if(!$Validation->isPrice($t[2],true)) continue;
			
			if($Date->compareTime($t[0],$t[1])!=2) continue;
			
			array_push($time,array('start'=>$t[0],'stop'=>$t[1],'price'=>CHBSPrice::formatToSave($t[2])));
		}		
		...
		{
			$t=array($value,$option['distance_base_location']['stop'][$index],$option['distance_base_location']['price'][$index]);
	...
	function manageEditColumns($column)
	{
		...
		(
			'cb'=>$column['cb'],
			'title'=>$column['title'],
			'rule'=>__('Conditions','chauffeur-booking-system'),
				...
				if((isset($meta['distance_base_location'])) && (count($meta['distance_base_location'])))
				{
					foreach($meta['distance_base_location'] as $value)
					{
						...
						if(in_array($meta['price_source_type'],array(4,5)))
							$html['distance'].=' ('.CHBSPrice::format($value['price'],CHBSOption::getOption('currency')).')';
					}
				}   
	...
	function getPriceFromRule($bookingData,$bookingForm,$priceRule=array())
	{
		...
		foreach($rule as $ruleData)
		{
			...
			if(is_array($ruleData['meta']['distance_base_location']))
			{
				...
				if((int)$ruleData['meta']['price_source_type']===4)
				{
					...
					if($bookingData['base_location_distance']>0)
					{
						...
						{
							foreach($ruleData['meta']['base_location_distance'] as $value)
							{
								if(($value['start']<=$i) && ($value['stop']>=$i))
								{
									$sum+=($value['price']/10);
			...
			{
				if(count($ruleData['meta']['duration']))
				{
