	...
	function calculatePrice($data,$calculateHiddenFee=true,$roundVehiclePrice=false,$bidData=array())
	{
		...
		else
		{
			...
			if(in_array($data['service_type_id'],array(1,3)))
			{
				...
				{
					$priceSumNetValuePart[0]=$priceBase['price_distance_value']*$distance;					
					$priceSumGrossValue+=number_format($priceSumNetValuePart[0]*(1+$TaxRate->getTaxRateValue($priceBase['price_distance_tax_rate_id'],$taxRate)/100),2,'.','');  				
				}
			
				if(in_array($calculationMethod,array(2,3,7)))
				{
					$priceSumNetValuePart[1]=$priceBase['price_hour_value']*$duration;
					$priceSumGrossValue+=number_format($priceSumNetValuePart[1]*(1+$TaxRate->getTaxRateValue($priceBase['price_hour_tax_rate_id'],$taxRate)/100),2,'.',''); 
				}
				...
				if(in_array((int)$data['transfer_type_id'],array(2,3)))
				{
					...
					if(in_array($calculationMethod,array(1,2,3,7)))
					{
						$priceSumNetValuePart[0]=$priceBase['price_distance'.$priceType.'_value']*$distance;	
						$priceSumGrossValue+=number_format($priceSumNetValuePart[0]*(1+$TaxRate->getTaxRateValue($priceBase['price_distance'.$priceType.'_tax_rate_id'],$taxRate)/100),2,'.','');  				
					}

					if(in_array($calculationMethod,array(2,3,7)))
					{
						$priceSumNetValuePart[1]=$priceBase['price_hour'.$priceType.'_value']*$duration;
						$priceSumGrossValue+=number_format($priceSumNetValuePart[1]*(1+$TaxRate->getTaxRateValue($priceBase['price_hour'.$priceType.'_tax_rate_id'],$taxRate)/100),2,'.',''); 
					}
				...
				if(in_array($calculationMethod,array(4,7)))
				{
					...
					if($passengerAdult)
					{
						...
						$priceSumGrossValue+=number_format($priceSumNetValuePart[0]*(1+$TaxRate->getTaxRateValue($priceBase['price_passenger_adult_tax_rate_id'],$taxRate)/100),2,'.','');					 
					}

					if($passengerChildren)
					{
						$priceSumNetValuePart[1]=$priceBase['price_passenger_children_value']*$passengerChildren*$returnFactor; 
						$priceSumGrossValue+=number_format($priceSumNetValuePart[1]*(1+$TaxRate->getTaxRateValue($priceBase['price_passenger_children_tax_rate_id'],$taxRate)/100),2,'.','');					 
					}	
			...
			foreach($vehicleValue['meta']['attribute'] as $vehicleAttributeIndex=>$vehicleAttributeValue)
			{
				...
				switch($dictionary[$vehicleAttributeIndex]['meta']['attribute_type'])
				{
					...
					case 3:
						...
						{
							if(is_array($dictionary[$vehicleAttributeIndex]['meta']['attribute_value']))
							{
