...
class CHBSBooking
{
	...
	function sendBooking($data,$bookingForm)
	{
		...
		{
			$bookingOld=$this->getBooking($bookingForm['booking_edit']->bookingId);
		
			/***/
		
			CHBSPostMeta::updatePostMeta($bookingForm['booking_edit']->bookingI,'booking_status_id',3); 

			/***/
		
			$bookingNew=$this->getBooking($bookingForm['booking_edit']->bookingId);
		
			$emailSend=false;
		
			$WooCommerce=new CHBSWooCommerce();
			$WooCommerce->changeStatus(-1,$bookingForm['booking_edit']->bookingId,$emailSend);
		...
		if(in_array($bookingForm['meta']['customer_sms_enable'],array(1,2)))
		{
			if(($Validation->isNotEmpty($booking['meta']['client_contact_detail_phone_number'])) && ($Validation->isNotEmpty($bookingForm['meta']['customer_sms_message'])))
			{
				...
				{
					if($i===0) $pickupLocation=$value['formatted_address'];
					$dropoffLocation=$value['formatted_address'];
					$i++;
				}
				...
				$replacement[5]=CHBSPrice::format($booking['billing']['summary']['value_gross'],$booking['meta']['currency_id']);
	...
	function calculatePrice($data,$vehiclePrice=null,$hideFee=false,$roundVehiclePrice=false)
	{
		...
		if(($Payment->isPayment($data['payment_id'])) && (in_array($data['payment_id'],array(2,3))))
		{
			...
			{				
				$sum=number_format($price['total']['sum']['gross']['value']*(1+($vehiclePrice['price']['base']['price_payment_'.$type.'_percentage_value']/100)),2,'.','');
