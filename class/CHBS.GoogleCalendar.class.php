...
class CHBSGoogleCalendar
{
	...
	function sendBooking($bookingId,$bookingReturn=false,$action='after_booking_send')
	{
		...
		/*
		$bookingTitle.=$booking['post']->post_title.', '.sprintf(__('Customer: %s','chauffeur-booking-system'),$booking['meta']['client_contact_detail_first_name'].' '.$booking['meta']['client_contact_detail_last_name']);
		$bookingTitle.=', '.sprintf(__('Route: %s','chauffeur-booking-system'),$route);
		if(array_key_exists('vehicle_passenger_count',$data['booking']))
			$bookingTitle.=', '.sprintf(__('Vehicle passengers count: %s','chauffeur-booking-system'),$data['booking']['vehicle_passenger_count']);
		if(array_key_exists('vehicle_bag_count',$data['booking']))
			$bookingTitle.=', '.sprintf(__('Vehicle bag count: %s','chauffeur-booking-system'),$data['booking']['vehicle_bag_count']);
		*/
